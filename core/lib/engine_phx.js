...
module.exports = PhxEngine;

function PhxEngine(script) {
    this.config = script.config;
}

PhxEngine.prototype.createScenario = function (scenarioSpec, ee) {
    var self = this;
...
PhxEngine.prototype.step = function (requestSpec, ee) {
    let self = this;
...
PhxEngine.prototype.compile = function (tasks, scenarioSpec, ee) {
    ...
    return function scenario(initialContext, callback) {
        function zero(callback) {
            ...
            let ws = new WebSocket(config.target + `?vsn=2.0.0&token=${initialContext.vars.auth_token}`, subprotocols, options);
